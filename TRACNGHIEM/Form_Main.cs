...
namespace TRACNGHIEM
{
    public partial class Form_Main : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void btn_Restore_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            String strLenh = "RESTORE DATABASE TRACNGHIEM FROM DISK='D:\\backup\\tracnghiem.bak' WITH REPLACE";
            try
            {
                ...
                Program.myReader.Close();
                Program.conn.Close();
                MessageBox.Show("RS   thành công", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (SqlException ex)
            {
                MessageBox.Show("Không thể RS\nLý do: " + ex.Message, "", MessageBoxButtons.OK);
            }
        ...
        {
            Form frm = this.CheckExists(typeof(Form_SoanCH));
            if (frm != null) frm.Activate();
            else
            {
                Form_SoanCH f = new Form_SoanCH();
                f.MdiParent = this;
                f.Show();
            }
        }

        private void btn_thitn_ItemClick(object sender, ItemClickEventArgs e)
        {
            Form frm = this.CheckExists(typeof(Form_ThiTN));
            if (frm != null) frm.Activate();
            else
            {
                Form_ThiTN f = new Form_ThiTN();
                f.MdiParent = this;
                f.Show();
            }
        }
